...
import { analyzePlant, explainPlantDisease } from './services/geminiService';
...
export default function App() {
  ...
  const [view, setView] = useState<'home' | 'detect' | 'history' | 'settings'>('home');
  const [authMode, setAuthMode] = useState<'login' | 'register'>('login');
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [scanResult, setScanResult] = useState<ScanResult | null>(null);
  const [history, setHistory] = useState<ScanHistoryItem[]>([]);
  const [capturedImage, setCapturedImage] = useState<string | null>(null);
  const [diseaseExplanation, setDiseaseExplanation] = useState<string | null>(null);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      if (data) {
        localStorage.setItem('scan_history', JSON.stringify(data.slice(0, 20)));
      }
    } catch (err: any) {
      console.error("Failed to fetch history from Supabase", err);
      setHistoryError(err.message === 'Failed to fetch' ? "Network error: Could not reach server." : "Failed to load history.");
      // Fallback to local storage
      const localHistory = localStorage.getItem('scan_history');
      if (localHistory) {
        try {
      ...
      try {
        ...
        if (!result.unclearImage) {
          ...
          try {
            ...
            console.error("Failed to save scan to Supabase", supabaseErr);
          }

          // Always save to local storage as fallback/cache
          const localHistory = JSON.parse(localStorage.getItem('scan_history') || '[]');
          const updatedHistory = [newScan, ...localHistory].slice(0, 20);
          localStorage.setItem('scan_history', JSON.stringify(updatedHistory));
          
          if (view === 'history') {
            setHistory(updatedHistory as any);
          }
        }
      } catch (err: any) {
        ...
        try {
          // Attempt to parse JSON error from Gemini API
          const parsedError = JSON.parse(err.message);
          if (parsedError.error?.message?.includes("API key not valid")) {
            errorMessage = "Invalid Gemini API Key. Please check your API key in the AI Studio Settings menu.";
          } else {
          ...
          if (err.message?.includes("API key not valid") || err.message?.includes("API_KEY_INVALID")) {
            errorMessage = "Invalid Gemini API Key. Please check your API key in the AI Studio Settings menu.";
          } else if (err.message?.includes("GEMINI_API_KEY is missing")) {
            errorMessage = "Gemini API Key is missing. Please add it to your environment variables in the Settings menu.";
          } else {
  ...
  const handleExplain = async () => {
    ...
    try {
      const explanation = await explainPlantDisease(language);
      setDiseaseExplanation(explanation);
    } catch (err) {
      console.error("Failed to explain disease", err);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-emerald-50 font-sans text-slate-900 pb-20">
        ...
        <AnimatePresence mode="wait">
          {view === 'home' && (
            ...
            >
              <div className="bg-white rounded-3xl p-6 shadow-md border border-emerald-100">
                <h2 className="text-lg font-bold mb-1">Welcome, {user?.name || 'Farmer'}!</h2>
                <div className="flex items-center justify-between">
                  <p className="text-slate-500 text-sm">{t.tagline}</p>
                  <button 
                    onClick={() => setView('settings')}
                    className="p-2 bg-slate-50 rounded-xl text-slate-400 hover:text-emerald-600 transition-colors"
                  >
                    <Settings className="w-5 h-5" />
                  </button>
                </div>
              ...
              <div className="bg-white rounded-3xl p-6 shadow-md border border-emerald-100">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold">Learn About Plant Diseases</h3>
                </div>
                {diseaseExplanation ? (
                  <div className="space-y-4">
                    <p className="text-sm text-slate-600 leading-relaxed">{diseaseExplanation}</p>
                    <button 
                      onClick={() => setDiseaseExplanation(null)}
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-emerald-50 text-emerald-700 font-bold rounded-xl border border-emerald-100 flex items-center justify-center gap-2"
                  >
                    {isExplaining && <Loader2 className="w-4 h-4 animate-spin" />}
                    ...
                    <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-start gap-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-4">
                          ...
                          {(analysisError.includes("API Key") || analysisError.includes("GEMINI_API_KEY")) && (
                            <button 
                              onClick={() => setView('settings')}
                              className="text-xs font-bold text-emerald-600 underline"
                            >
          ...
          {view === 'history' && (
            ...
            >
              ...
              {historyError && (
                <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-center justify-between mb-4">
            ...
            >
              ...
              <div className="bg-white rounded-3xl p-6 shadow-md border border-emerald-100">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="font-bold">{user?.name || 'Farmer'}</h3>
                    <p className="text-slate-500 text-sm">{user?.email || 'Guest User'}</p>
                  </div>
                </div>
                  ...
                  <div className="pt-6 border-t border-slate-100">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="font-bold text-sm uppercase tracking-wider text-slate-400">Gemini API Key (Manual)</h4>
                      </div>
                      <button 
                    ...
                    {showKeyInput ? (
                      <div className="space-y-3">
                        ...
                        <button 
                          ...
                          className="w-full py-3 bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-200 active:scale-95 transition-transform"
                        >
                          Save Key
                        </button>
                        <p className="text-[10px] text-slate-400 text-center">
                          Your key is saved only in your browser's memory.
                        </p>
                      </div>
                    ) : (
                      <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between">
                        <span className="text-sm text-slate-500">
                          {manualKey ? '••••••••••••' + manualKey.slice(-4) : 'Not Configured'}
                        </span>
                        <div className={cn(
                          "w-2 h-2 rounded-full",
                          manualKey ? "bg-emerald-500" : "bg-slate-300"
                        )} />
                      </div>
                    )}
                  </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 px-6 py-3 flex justify-around items-center z-50">
        ...
        <button 
          onClick={() => setView('settings')}
          className={cn("p-2 rounded-xl transition-colors", view === 'settings' ? "text-emerald-600 bg-emerald-50" : "text-slate-400")}
        >
