...
const getAI = () => {
  // Check local storage first for manual key
  ...
  const apiKey = 
    ...
    "AIzaSyBuSjTFvRIIfyEbAeCAZGjOltMMa_IdQ2A";
    
  if (!apiKey || (typeof apiKey === 'string' && apiKey.trim() === "")) {
    throw new Error("GEMINI_API_KEY is missing. Please set it in your environment variables via the Settings menu or enter it manually in the app settings.");
  }
...
export async function explainPlantDisease(language: Language): Promise<string> {
  const ai = getAI();
  const langMap: Record<Language, string> = {
    en: "English",
    hi: "Hindi",
  ...
  const prompt = `Explain what plant leaf disease is in very simple, farmer-friendly words in ${langMap[language]}. 
  Use short sentences. No scientific jargon. Imagine you are talking to a farmer who wants to protect their crops.`;
